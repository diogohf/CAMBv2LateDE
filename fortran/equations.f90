    ...
    type(EvolutionVars) EV
    integer nu_i, neq
    integer, intent(out), optional :: maxeq
    neq = 2
    ...
    if (CP%WantScalars) then
        ...
        else if (AccuracyTarget > 0 .and. CP%Want_CMB .and. EV%q > 0.15_dl) then
        ...
        if (EV%q < 0.05) then
            ...
            else if (CP%Accuracy%AccurateReionization) then
        ...
        if (CP%Transfer%high_precision .or. CP%Do21cm) then
            ...
            else if (EV%q >= 0.05_dl .and. EV%q < 0.12_dl) then
                transfer_lmaxnr = (14._dl + 31._dl*(EV%q - 0.05_dl)/0.07_dl)*l_accuracy_boost
                EV%lmaxnr = max(EV%lmaxnr, nint(transfer_lmaxnr), min_lmaxnr)
            else if (EV%q >= 0.12_dl) then
    ...
    subroutine MassiveNuVarsOut(EV, y, yprime, a, adotoa, grho, gpres, dgrho, dgq, dgpi, dgpi_diff, pidot_sum, &
        clxnu_all)
    implicit none
    type(EvolutionVars) EV
    ...
    do w_ix = 1, State%num_redshiftwindows
        associate(w => State%Redshift_W(w_ix))
            ...
            if (w%kind == window_lensing) then
            ...
            else if (w%kind == window_counts) then
            ...
            else if (w%kind == window_21cm) then
                ...
                if (CP%SourceTerms%line_extra) then
                    ...
                    if (CP%SourceTerms%line_basic .and. CP%SourceTerms%line_distortions) then
                        sources(3 + w_ix) = (-2.d0/3.d0*sigma + 2.d0/3.d0*etak/adotoa)*w%winV(j)*exptau*k + &
                            ...
                            exptau + ((-w%dwing(j)*vb + &
                            (-(3.d0*gpres + grho)*sigma/3.d0 - 4.d0*adotoa**2*sigma)*w%winV(j) + &
                            4.d0*adotoa*w%dwinV(j)*sigma + (-sigma - vb)*w%ddwinV(j) - vbdot*w%wing(j) - &
                            w%dwinV(j)*vbdot + (-2.d0*w%dwinV(j)*etak + 2.d0*etak*adotoa*w%winV(j))/EV%Kf(1))*exptau - &
                            2.d0*visibility*sigma*w%dwinV(j) + &
                            (4.d0*visibility*sigma*adotoa - dvisibility*sigma)*w%winV(j) - &
                            2.d0*visibility*w%winV(j)*etak/EV%Kf(1) - visibility*w%dwinV(j)*vb - &
                            visibility*w%wing(j)*vb)/k + &
                            ((2.d0*w%dwinV(j)*dgpi + diff_rhopi*w%winV(j))*exptau + 2.d0*visibility*w%winV(j)*dgpi)/k**2
                    else
                        ...
                        s(5) = s(6) + (-2.d0*visibility*etak/adotoa*w%wing(j) - 2.d0*visibility*w%winV(j)*etak)/ &
                            EV%Kf(1) + (4.d0*visibility*sigma + &
                            (-visibility*grho*sigma/3.d0 - visibility*vb*grho/3.d0)/adotoa**2 + visibility*vb + &
                            (-dvisibility*sigma - dvisibility*vb)/adotoa)*w%wing(j) + &
                ...
                if (CP%SourceTerms%line_reionization) then
                    ...
                    s(4) = (-1.d0/EV%Kf(1)*visibility*w%winV(j)*etak/10.d0 - visibility*sigma*w%dwinV(j)/10.d0 - &
                        ...
                        9.d0/10.d0*dvisibility*y(lineoff + 3) - &
                        3.d0/20.d0*visibility*opacity*EV%Kf(2)*y(lineoffpol + 3) + visibility*w%dwinV(j)*vb + &
                        81.d0/200.d0*visibility*opacity*y(lineoff + 3) + 3.d0/5.d0*dvisibility*y(lineoff + 1) + &
                        3.d0/10.d0*visibility*yprime(lineoff + 1) + &
                    ...
                    s(5) = (visibility*w%winV(j)*dgpi/10.d0 + 9.d0/20.d0*visibility*dopacity*y(lineoffpol + 2) + &
                        ...
                        117.d0/200.d0*visibility*opacity**2.d0*y(lineoffpol + 2) + &
                        3.d0/4.d0*ddvisibility*y(lineoff + 2) - 27.d0/20.d0*dvisibility*opacity*y(lineoff + 2) + &
                        9.d0/10.d0*dvisibility*opacity*y(lineoffpol + 2) - &
                ...
                if (CP%SourceTerms%line_phot_dipole) then
                    sources(3 + w_ix) = sources(3 + w_ix) + &
                        (EV%Kf(1)*w%wing2(j)*pig/2.d0 - w%wing2(j)*clxg/4.d0)*exptau + &
                        (((vbdot - opacity*vb + 3.d0/4.d0*opacity*qg)*w%wing2(j) + (vb - 3.d0/4.d0*qg)*w%dwing2(j))* &
                        exptau + (visibility*vb - 3.d0/4.d0*visibility*qg)*w%wing2(j))/k
                end if
    ...
    ! DHFS NOTE: 
    ! CAMBLateDE has this commented out.
    ! This time we want perturbations.
        ...
        if (a > State%CP%Recomb%min_a_evolve_Tm) then
            if (State%CP%SourceTerms%line_reionization) then
                ...
                if (tau > EV%ThermoData%tau_start_redshiftwindows) then
                    ...
                    Delta_source2 = &
                        Get21cm_source2(a, clxb, Delta_TCMB, Delta_TM, Delta_xe, Tmat, Trad, xe, k*(z + vb)/adotoa/3)
                    ...
                    ! dipole
                    ! vb/3*WinV_t)
                    ayprime(lineoff + 1) = EV%denlk(1)*ay(lineoff) - EV%denlk2(1)*ay(lineoff + 2) - &
                        opacity*ay(lineoff + 1) - wing2_t*(qg/4 - vb/3)
                    ...
                    ayprime(lineoffpol + EV%lmaxline) = -opacity*ay(lineoffpol + EV%lmaxline) + &
                        k*EV%poltruncfac*ay(lineoffpol + EV%lmaxline - 1) - &
                        (EV%lmaxline + 3)*cothxor*ay(lineoffpol + EV%lmaxline)
                else
    ...
    if (State%CP%Num_Nu_massive > 0) then
        ...
        do nu_i = 1, State%CP%Nu_mass_eigenstates
            if (EV%MassiveNuApprox(nu_i)) then
                ...
                ayprime(off_ix + 2) = &
                    (3*w - 1)*adotoa*ay(off_ix + 2) - k*(2._dl/3*EV%Kf(1)*ay(off_ix + 3) - ay(off_ix + 1))
                ...
                if (.not. EV%EvolveTensorMassiveNu(nu_i)) then
                    ...
                    do i = 1, State%NuPerturbations%nqmax
                        ...
                        if (EV%lmaxnut > 2) then
                            ...
                            ! Truncate moment expansion.
                            aytprime(ind) = &
                                k*v*EV%lmaxnut/(EV%lmaxnut - 2._dl)*ayt(ind - 1) - (EV%lmaxnut + 3)*cothxor*ayt(ind)
                        else
