    ...
    type(TBispectrumParams), intent(in), optional :: BispectrumConfig
    type(TBispectrumResult), optional :: BispectrumResult
    character(len=*), intent(in), optional :: bispectrum_output_root
    type(CAMBParams) P
    ...
    do i = 1, num_redshiftwindows
        ...
        class is (TGaussianSourceWindow)
            ...
            else if (S == 'counts') then
                RedWin%source_type = window_counts
            else if (S == 'lensing') then
    ...
    DarkEneryModel = UpperCase(Ini%Read_String_Default('dark_energy_model', 'ppf'))
    ...
    if (((P%NonLinear == NonLinear_Lens .or. P%NonLinear == NonLinear_both) .and. P%DoLensing) .or. &
        PK_WantTransfer) then
    ...
    ! Read this even if do_nonlinear=0, so that the settings survive a write/read round trip
    ...
    character(len=*), intent(in), optional :: IniName
